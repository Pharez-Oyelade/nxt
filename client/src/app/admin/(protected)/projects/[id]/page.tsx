...
import { useGetProject, useUpdateProject, useUploadDeliverable } from "@/hooks/useProjects";
import { Loader2, ArrowLeft, Building2, UploadCloud, FileIcon, ExternalLink } from "lucide-react";
  ...
  if (error || !project) {
    return (
      <div className="flex-1 p-8 flex flex-col items-center justify-center min-h-[400px]">
        <p className="text-destructive font-medium mb-4">Failed to load project details.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              {project.files && project.files.length > 0 ? (
                project.files.map((file) => (
                  <div key={file._id} className="flex items-center justify-between p-3 rounded-xl border border-border/50 bg-muted/30 group hover:border-accent/40 transition-colors">
                    <div className="flex items-center gap-3 overflow-hidden">
                      ...
                      <div className="overflow-hidden">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {file.uploadedAt ? format(new Date(file.uploadedAt), "MMM d, yyyy h:mm a") : "Recently"}
                        </p>
                      </div>
                    </div>
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center border-2 border-dashed border-border/50 rounded-xl bg-muted/10">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">No deliverables uploaded yet</p>
                  <p className="text-xs text-muted-foreground mt-1">Upload project files, PDFs, or images here.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <Link href={`/admin/clients/${project.clientId._id}`} className="font-semibold text-primary hover:underline block">
          ...
          <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
               <div>
                 <p className="text-xs text-muted-foreground uppercase font-semibold tracking-wider mb-1">Created</p>
                 <p className="text-sm font-medium">
                   {format(new Date(project.createdAt), "MMMM d, yyyy")}
                 </p>
               </div>
               <div>
                 <p className="text-xs text-muted-foreground uppercase font-semibold tracking-wider mb-1">Last Updated</p>
                 <p className="text-sm font-medium">
